...
const NavigationLink = ({ to, children, linkClassName }) => (
	<li className={`md:ml-8 text-base md:my-0 my-7`}>
	  <Link to={to} className={`text-offwhite hover:text-gray-400 duration-500 ${linkClassName}`}>
		{children}
	  </Link>
	</li>
  );
...
const Nav = () => {
	...
	return (
		<div className=' shadow-md w-full fixed top-o left-0'>
			<div className='md:flex items-center justify-between bg-primary py-2 md:px-10 px-7'>
				...
				<div
					...
					className='text-3xl absolute right-8 top-6 cursor-pointer md:hidden text-offwhite'
				>
